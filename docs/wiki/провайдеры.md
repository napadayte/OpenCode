...
> [!note]
> Нет ключа `"servers"`. Серверы идут прямо в `"mcp"`. Тип — `"local"` (не `"stdio"`). Команда — массив.
...
## MCP в глобальном конфиге

Глобальный `opencode.json` (репо) подключает по умолчанию:

| Сервер | Что даёт | Команда |
|---|---|---|
| `playwright` | Браузер: открыть страницу, кликнуть, скриншот, заполнить форму | `npx @playwright/mcp@latest` |

Требует Node.js (`npx`). При первом запуске Playwright скачает браузеры (~200 МБ).
Чтобы выключить — поменяй `enabled: false` или удали блок.
...
model: anthropic/claude-opus-4-5   # claude
---
```
