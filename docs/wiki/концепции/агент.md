...
> - **`tools: - Read - Edit`** (массив) — это формат Claude Code. В OpenCode для отключения конкретных инструментов используется **map**: `tools: { write: false, edit: false }`. А для тонкого контроля разрешений — блок `permission:`.
