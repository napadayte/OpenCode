...
> **Status (2026-05).** Реализовано: OpenCode (primary), Claude Code, Codex, Gemini. Секции про Aider, Cursor, Cline, Continue — **аспирационные** (документация формата на будущее, адаптеров в репо ещё нет). Phase-чеклист в конце файла отражает первоначальный план внедрения и сохранён как исторический контекст — актуальное состояние смотри в `docs/wiki/` и `AGENTS.md` шаблонов.
...
Правило:
    ...
    сначала plan
    потом маленькая реализация вручную
    потом review
...
Tasks:
    ...
    - [ ] Create ~/.config/opencode/projects/templates/agent-ready-project
    - [ ] Add AGENTS.md
