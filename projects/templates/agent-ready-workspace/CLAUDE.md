...
If you want Claude's agent / skill picker UI to list the workspace's agents and skills, mirror them via symlinks or copy into `.claude/agents/` and `.claude/skills/`. Note: Claude Code uses different frontmatter (`name`, `model: sonnet|opus|haiku`, `tools: [...]`) — see `.opencode/agents/README.md` for details.
